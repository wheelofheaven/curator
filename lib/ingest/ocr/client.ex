defmodule Ingest.OCR.Client do
  @moduledoc """
  ...
  def extract(pdf_path, opts \\ []) do
    api_key = opts[:api_key] || Application.get_env(:ingest, :zai_api_key)
  ...
  def extract_url(pdf_url, opts \\ []) do
    ...
    case Req.post(@api_url,
      ...
      {:ok, %Req.Response{status: 200, body: body}} ->
        Ingest.OCR.Response.parse(body)
  ...
  defp extract_chunked(pdf_path, total_pages, api_key, opts) do
    ...
    tmp_dir = Path.join(System.tmp_dir!(), "ingest_chunks_#{System.unique_integer([:positive])}")
