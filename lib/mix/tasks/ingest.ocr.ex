defmodule Mix.Tasks.Ingest.Ocr do
  @moduledoc """
  ...
  ## Usage
      ...
      mix ingest.ocr path/to/book.pdf --slug my-book

  If a sidecar JSON exists alongside the PDF, the slug is loaded automatically:

      mix ingest.ocr path/to/book.pdf
  """
  ...
  def run(args) do
    ...
    pdf_path =
      case positional do
        [path | _] -> path
        [] -> Mix.raise("Usage: mix ingest.ocr <pdf_path> [--slug <slug>]")
      end
    ...
    if Ingest.Stages.OCR.done?(slug) && !opts[:force] do
      Mix.shell().info("OCR output already exists for '#{slug}'. Use --force to re-extract.")
      Mix.shell().info("  Path: #{Ingest.Stages.WorkDir.artifact(slug, "ocr.md")}")
