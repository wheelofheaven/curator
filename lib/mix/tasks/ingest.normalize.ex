defmodule Mix.Tasks.Ingest.Normalize do
  @moduledoc """
  ...
  ## Usage
      ...
      mix ingest.normalize --slug my-book --code MYBOOK --lang fr --rules raelian

  If a sidecar JSON exists for the original PDF, metadata is loaded automatically:

      mix ingest.normalize --slug my-book
  ...
  def run(args) do
    ...
    unless Ingest.Stages.OCR.done?(slug) do
      Mix.raise("No OCR output found for '#{slug}'. Run `mix ingest.ocr` first.")
    end

    if Ingest.Stages.Normalize.done?(slug) && !opts[:force] do
      Mix.shell().info("Normalized output already exists for '#{slug}'. Use --force to re-run.")
      Mix.shell().info("  Path: #{Ingest.Stages.WorkDir.artifact(slug, "book.json")}")
      ...
      case Ingest.Stages.Normalize.run(slug, metadata, normalize_opts) do
        {:ok, result} ->
          book = result.book
          stats = Ingest.Refiner.Strategy.refinement_stats(book)

          Mix.shell().info("Normalization complete!")
          Mix.shell().info("  Output: #{result.path}")
          Mix.shell().info("  Chapters: #{Ingest.Schema.Book.chapter_count(book)}")
          Mix.shell().info("  Paragraphs: #{stats.total}")
          Mix.shell().info("  Confidence: #{stats.percentage_clear}% clear")
          Mix.shell().info("")
          Mix.shell().info("Next step: mix ingest.translate --slug #{slug}")
  ...
  defp return_existing(slug) do
    case Ingest.Stages.Normalize.load(slug) do
      {:ok, json} ->
