defmodule Mix.Tasks.Ingest.Translate do
  @moduledoc """
  ...
  ## Usage
      ...
      mix ingest.translate --slug my-book
      mix ingest.translate --slug my-book --langs en,de,es
  ...
  def run(args) do
    ...
    unless Ingest.Stages.Normalize.done?(slug) do
      Mix.raise("No normalized output found for '#{slug}'. Run `mix ingest.normalize` first.")
    end

    if Ingest.Stages.Translate.done?(slug) && !opts[:force] do
      Mix.shell().info("Translated output already exists for '#{slug}'. Use --force to re-run.")
      Mix.shell().info("  Path: #{Ingest.Stages.WorkDir.artifact(slug, "book_translated.json")}")
    else
      langs =
        case opts[:langs] do
          nil -> Application.get_env(:ingest, :default_languages, ~w(en fr de es ru ja zh))
      ...
      case Ingest.Stages.Translate.run(slug, translate_opts) do
        {:ok, result} ->
          Mix.shell().info("Translation complete!")
          Mix.shell().info("  Output: #{result.path}")
          Mix.shell().info("")
          Mix.shell().info("Next step: mix ingest.export --slug #{slug}")
