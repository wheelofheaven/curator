defmodule Mix.Tasks.Ingest.Run do
  ...
  For more control, use the individual stage tasks:
    ...
    mix ingest.ocr        — Extract text from PDF
    mix ingest.normalize   — Parse into structured JSON
    mix ingest.translate   — Translate into target languages
    mix ingest.export      — Write to data-library format
  ...
  metadata is loaded automatically:
      ...
      mix ingest.run path/to/book.pdf
  ...
  @shortdoc "Run full source document ingestion pipeline (OCR → Normalize → Export)"
  ...
  def run(args) do
    ...
    pdf_path =
      case positional do
        [path | _] -> path
        [] -> Mix.raise("Usage: mix ingest.run <pdf_path> [options]")
      end
    ...
    case Ingest.Pipeline.run(pdf_path, metadata, pipeline_opts) do
      {:ok, result} ->
        ...
        Ingest.Stages.WorkDir.list_artifacts(slug)
