defmodule IngestWeb.DashboardLive do
  ...
  alias Ingest.Pipeline
  alias Ingest.Stages
  alias Ingest.Sources

  @impl true
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Ingest.PubSub, "jobs")
    end
      ...
      if source do
        ...
        uploaded_files =
          consume_uploaded_entries(socket, :pdf, fn %{path: path}, _entry ->
            dest = Path.join(System.tmp_dir!(), "ingest_#{System.unique_integer([:positive])}.pdf")
            File.cp!(path, dest)
            {:ok, dest}
          end)
  ...
  def render(assigns) do
    ...
    <IngestWeb.Layouts.app flash={@flash}>
      <div class="space-y-8">
        <div>
          <h1 class="text-2xl font-bold">Ingest Dashboard</h1>
          <p class="text-base-content/60 mt-1">
            Ingest source documents into the data-library structured JSON format.
          </p>
